import React from 'react';
import { Link } from 'react-router-dom';
import "../../assets/css/App.css";
import ellipse from "../../assets/images/ellipse-25.png";
const Conversa = () => {
  return (
    <div>

      <div class="mensagens">
        <div class="rectangle-1"></div>
        <svg class="rectangle-4" width="871" height="1000" viewBox="0 0 871 1000" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path d="M0 0H871V1000H0V0Z" fill="#3F51B5" />
        </svg>

        <div class="rectangle-21"></div>
        <div class="rectangle-22"></div>
        <div class="rectangle-57"></div>
        <div class="digite-uma-mensagem">digite uma mensagem...</div>
        <div class="rectangle-7">
          <div class="rectangle-6">
            <div class="rectangle-5"></div>
          </div>
        </div>
        <div class="rectangle-8">
          <div class="rectangle-6">
            <div class="rectangle-5"></div>
          </div>
        </div>
        <div class="rectangle-9">
          <div class="rectangle-6">
            <div class="rectangle-5"></div>
          </div>
        </div>
        <div class="rectangle-11">
          <div class="rectangle-6">
            <div class="rectangle-5"></div>
          </div>
        </div>
        <div class="rectangle-12">
          <div class="rectangle-6">
            <div class="rectangle-5"></div>
          </div>
        </div>
        <div class="rectangle-13">
          <div class="rectangle-6">
            <div class="rectangle-5"></div>
          </div>
        </div>
        <div class="rectangle-14">
          <div class="rectangle-6">
            <div class="rectangle-5"></div>
          </div>
        </div>
        <div class="rectangle-10">
          <div class="rectangle-6">
            <div class="rectangle-5"></div>
          </div>
        </div>
        <img src={ellipse} class="ellipse-9" />
        <div class="family">chat</div>
        <div class="_7-pessoas">7 pessoas</div>
        <div class="_17-50">17:50</div>
        <img src={ellipse} class="ellipse-10" /><img src={ellipse} class="ellipse-13" /><img src={ellipse} class="ellipse-12" /><img src={ellipse} class="ellipse-11" />
        <div class="family2">chat</div>
        <div class="_7-pessoas2">7 pessoas</div>
        <div class="_17-502">17:50</div>
        <div class="family3">chat</div>
        <div class="_7-pessoas3">7 pessoas</div>
        <div class="_17-503">17:50</div>
        <img src={ellipse} class="ellipse-16" />
        <div class="family4">chat</div>
        <img src={ellipse} class="ellipse-15" />
        <div class="_7-pessoas4">7 pessoas<br /></div>
        <img src={ellipse} class="ellipse-14" />
        <div class="_17-504">17:50</div>
        <div class="family5">chat</div>
        <div class="_7-pessoas5">7 pessoas</div>
        <div class="_17-505">17:50</div>
        <div class="family6">chat</div>
        <div class="_7-pessoas6">7 pessoas</div>
        <div class="_17-506">17:50</div>
        <div class="family7">chat</div>
        <div class="_7-pessoas7">7 pessoas</div>
        <div class="_17-507">17:50</div>
        <div class="_7-pessoas8">7 pessoas</div>
        <img class="ellipse-17" src="ellipse-17.png" />
        <div class="family8">chat</div>
        <div class="_17-508">17:50</div>
        <div class="iconsax-linear-add"></div>
        <div class="text">
          <br />
        </div>
        <div class="rectangle-39"></div>
        <img src={ellipse} class="ellipse-18" />
        <div class="iconsax-linear-link-21">
          <Link to="/criarsala">
            <svg class="vector" width="12" height="11" viewBox="0 0 12 11" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M10.0601 0.940002C10.595 1.47324 11.0193 2.10679 11.3089 2.80434C11.5985 3.5019 11.7475 4.24974 11.7475 5.005C11.7475 5.76027 11.5985 6.50811 11.3089 7.20566C11.0193 7.90322 10.595 8.53677 10.0601 9.07C7.81009 11.31 4.17009 11.32 1.93009 9.07C-0.30991 6.82 -0.31991 3.18 1.93009 0.940002"
                stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
          </Link>

          <Link to="/criarSala">
            <svg class="vector2" width="13" height="11" viewBox="0 0 13 11" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M2.58996 10.41C0.249961 8.07001 0.249961 4.27001 2.58996 1.92001C4.92996 -0.429993 8.72996 -0.419993 11.08 1.92001C13.43 4.26001 13.42 8.06001 11.08 10.41"
              stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
          </Link>
          
        </div>
        <div class="x">x</div>
        <div class="rectangle-40">
          <div class="rectangle-62">
            <div class="rectangle-52"></div>
          </div>
        </div>
        <svg class="iconsax-linear-send-2" width="24" height="24" viewBox="0 0 24 24" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path
            d="M10.1099 13.65L13.6899 10.06M7.39993 6.32003L15.8899 3.49003C19.6999 2.22003 21.7699 4.30003 20.5099 8.11003L17.6799 16.6C15.7799 22.31 12.6599 22.31 10.7599 16.6L9.91993 14.08L7.39993 13.24C1.68993 11.34 1.68993 8.23003 7.39993 6.32003Z"
            stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
        </svg>

        <div class="iconsax-linear-happyemoji">
          <svg class="vector4" width="21" height="20" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M10.97 20C16.493 20 20.97 15.523 20.97 10C20.97 4.477 16.493 0 10.97 0C5.44697 0 0.969971 4.477 0.969971 10C0.969971 15.523 5.44697 20 10.97 20Z"
              stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
          </svg>

          <svg class="vector5" width="10" height="3" viewBox="0 0 10 3" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M0.209961 0C0.747443 0.772244 1.46381 1.40296 2.29792 1.83829C3.13202 2.27363 4.05909 2.50066 4.99996 2.5C6.97996 2.5 8.72996 1.51 9.78996 0"
              stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
          </svg>

          <div class="">. .</div>
        </div>
        <div class="rectangle-33"></div>
        <div class="chats">chats</div>
        <svg class="icon-plus" width="34" height="31" viewBox="0 0 34 31" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M26.6668 14.2084H18.2457V6.45835C18.2457 6.11578 18.0978 5.78724 17.8346 5.54501C17.5714 5.30277 17.2144 5.16669 16.8422 5.16669C16.47 5.16669 16.113 5.30277 15.8498 5.54501C15.5866 5.78724 15.4387 6.11578 15.4387 6.45835V14.2084H7.01764C6.64541 14.2084 6.28842 14.3444 6.02521 14.5867C5.76201 14.8289 5.61414 15.1574 5.61414 15.5C5.61414 15.8426 5.76201 16.1711 6.02521 16.4134C6.28842 16.6556 6.64541 16.7917 7.01764 16.7917H15.4387V24.5417C15.4387 24.8843 15.5866 25.2128 15.8498 25.455C16.113 25.6973 16.47 25.8334 16.8422 25.8334C17.2144 25.8334 17.5714 25.6973 17.8346 25.455C18.0978 25.2128 18.2457 24.8843 18.2457 24.5417V16.7917H26.6668C27.039 16.7917 27.396 16.6556 27.6592 16.4134C27.9224 16.1711 28.0703 15.8426 28.0703 15.5C28.0703 15.1574 27.9224 14.8289 27.6592 14.5867C27.396 14.3444 27.039 14.2084 26.6668 14.2084Z"
            fill="#D9D9D9" />
        </svg>

        <svg class="rectangle-35" width="514" height="43" viewBox="0 0 514 43" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path
            d="M0 10C0 4.47715 4.47715 0 10 0H504C509.523 0 514 4.47715 514 10V33C514 38.5228 509.523 43 504 43H10C4.47715 43 0 38.5228 0 33V10Z"
            fill="black" fill-opacity="0.76" />
        </svg>

        <div class="pesquisar">pesquisar...</div>
        <div class="family9">chat</div>
        <div class="rectangle-31"></div>
        <img src={ellipse} class="ellipse-19" />
        <svg class="rectangle-32" width="43" height="16" viewBox="0 0 43 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 12.3636L43 0V16H26.4615L0 12.3636Z" fill="#D9D9D9" />
        </svg>

        <div class="sla-nada-msm">sla nada msm</div>
        <div class="_17-509">17:50</div>
        <div class="fulano">fulano</div>
        <div class="icon-home-2">
          <svg class="group" width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M16.1553 21H4.84465C2.16837 21 0 18.8195 0 16.1403V8.89471C0 7.56486 0.820465 5.8928 1.87535 5.07144L7.14 0.964642C8.72233 -0.267394 11.2521 -0.326063 12.893 0.827749L18.9293 5.06167C20.0916 5.87325 21 7.61375 21 9.0316V16.1501C21 18.8195 18.8316 21 16.1553 21ZM8.0386 2.11846L2.77395 6.22526C2.08047 6.77283 1.46512 8.01465 1.46512 8.89471V16.1403C1.46512 18.0079 2.97907 19.5333 4.84465 19.5333H16.1553C18.0209 19.5333 19.5349 18.0177 19.5349 16.1501V9.0316C19.5349 8.09288 18.8609 6.79239 18.0893 6.26437L12.053 2.03046C10.9395 1.24821 9.10326 1.28732 8.0386 2.11846Z"
              fill="white" />
            <path
              d="M10.5 17.0887C10.0995 17.0887 9.76746 16.7563 9.76746 16.3554V13.422C9.76746 13.0211 10.0995 12.6886 10.5 12.6886C10.9005 12.6886 11.2326 13.0211 11.2326 13.422V16.3554C11.2326 16.7563 10.9005 17.0887 10.5 17.0887Z"
              fill="white" />
          </svg>
        </div>
        <div class="rectangle-142">
          <div class="rectangle-6">
            <div class="rectangle-53"></div>
          </div>
        </div>
        <svg class="icon-lock" width="16" height="23" viewBox="0 0 16 23" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M13.1515 11.3726H14.9927C15.4285 11.3726 15.7818 11.7545 15.7818 12.2256V21.8923C15.7818 22.3634 15.4285 22.7452 14.9927 22.7452H0.789088C0.353288 22.7452 0 22.3634 0 21.8923V12.2256C0 11.7545 0.353288 11.3726 0.789088 11.3726H2.63029M13.1515 11.3726V5.68631C13.1515 3.79088 12.0994 0 7.89088 0C3.68241 0 2.63029 3.79088 2.63029 5.68631V11.3726M13.1515 11.3726H2.63029"
            stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
        </svg>

        <svg class="icon-lock2" width="16" height="23" viewBox="0 0 16 23" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M13.1515 11.3726H14.9927C15.4285 11.3726 15.7818 11.7545 15.7818 12.2256V21.8923C15.7818 22.3634 15.4285 22.7452 14.9927 22.7452H0.789088C0.353288 22.7452 0 22.3634 0 21.8923V12.2256C0 11.7545 0.353288 11.3726 0.789088 11.3726H2.63029M13.1515 11.3726V5.68631C13.1515 3.79088 12.0994 0 7.89088 0C3.68241 0 2.63029 3.79088 2.63029 5.68631V11.3726M13.1515 11.3726H2.63029"
            stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
        </svg>

        <div class="rectangle-41"></div>
        <img src={ellipse} class="ellipse-20" />

        <svg class="rectangle-42" width="43" height="16" viewBox="0 0 43 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 12.3636L43 0V16H26.4615L0 12.3636Z" fill="#D9D9D9" />
        </svg>

        <div class="sla-nada-msm2">sla nada msm</div>
        <div class="_17-5010">17:50</div>
        <div class="ciclano">ciclano</div>
        <div class="rectangle-44"></div>
        <img src={ellipse} class="ellipse-21" />
        <svg class="rectangle-45" width="43" height="16" viewBox="0 0 43 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 12.3636L43 0V16H26.4615L0 12.3636Z" fill="#D9D9D9" />
        </svg>

        <div class="sla-nada-msm3">sla nada msm</div>
        <div class="_17-5011">17:50</div>
        <div class="criatura">criatura</div>
        <div class="rectangle-47"></div>
        <img src={ellipse} class="ellipse-22" />

        <svg class="rectangle-48" width="43" height="16" viewBox="0 0 43 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 12.3636L43 0V16H26.4615L0 12.3636Z" fill="#D9D9D9" />
        </svg>

        <div class="sla-nada-msm4">sla nada msm</div>
        <div class="_17-5012">17:50</div>
        <div class="humano">humano</div>
        <div class="rectangle-522"></div>
        <img src={ellipse} class="ellipse-23" />

        <svg class="rectangle-51" width="43" height="16" viewBox="0 0 43 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M43 12.3636L0 0L0 16L16.5385 16L43 12.3636Z" fill="#D9D9D9" />
        </svg>

        <div class="sla-nada-msm5">sla nada msm</div>
        <div class="_17-5013">17:50</div>
        <div class="you">you</div>
        <div class="rectangle-532"></div>

        <img src={ellipse} class="ellipse-24" />
        <svg class="rectangle-54" width="43" height="16" viewBox="0 0 43 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M43 12.3636L0 0L0 16L16.5385 16L43 12.3636Z" fill="#D9D9D9" />
        </svg>

        <div class="sla-nada-msm6">sla nada msm</div>
        <div class="_17-5014">17:50</div>
        <div class="you2">you</div>
        <div class="rectangle-55"></div>
        <img src={ellipse} class="ellipse-25" />

        <svg class="rectangle-56" width="43" height="16" viewBox="0 0 43 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M43 12.3636L0 0L0 16L16.5385 16L43 12.3636Z" fill="#D9D9D9" />
        </svg>

        <div class="sla-nada-msm7">sla nada msm</div>
        <div class="_17-5015">17:50</div>
        <div class="you3">you</div>

      </div>
    </div>
  );
};

export { Conversa }
